...
import { Box, Button, Card, CardContent, Chip, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, Grid, IconButton, LinearProgress, List, ListItem, ListItemText, Paper, Tab, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tabs, Typography } from '@mui/material';
import {
...
const SecurityDashboard = () => {
  ...
  <Grid item xs={12} md={3}>,
  <Card className={`${classes.securityCard} ${securityMetrics.systemHealth === 'healthy' ? classes.healthyCard : classes.warningCard}`}>,
  <CardContent>,
  <Box display="flex" alignItems="center" justifyContent="center" flexDirection="column">,
  <CheckIcon style={{ fontSize: 40, marginBottom: 16 }} />,
  <Typography className={`${classes.metricValue} ${securityMetrics.systemHealth === 'healthy' ? classes.healthyMetric : classes.warningMetric}`}>,
  {securityMetrics.systemHealth === 'healthy' ? '✓' : '!'}
  </Typography>,
  <Typography className={classes.metricLabel}>,
  System Health,
  </Typography>,
  ...
  label={threat.level}
  size="small",
  className={getSeverityClass(threat.level)}
  />,
  </Box>,
  ))}
  </CardContent>,
  ...
  {device.type === 'mobile' ? <PhoneIcon /> : <ComputerIcon />}
  <Box ml={2}>,
  <Typography variant="body2" fontWeight="bold">,
  {device.name}
  </Typography>,
  <Typography variant="caption" color="textSecondary">,
  {device.location} • Last seen: {formatTimestamp(device.lastSeen)}
  </Typography>,
  </Box>,
  </Box>,
  <Chip,
  label={device.status}
  size="small",
  color={device.status === 'trusted' ? 'success' : device.status === 'suspicious' ? 'warning' : 'error'}
  />,
  </Box>,
  </Box>,
  ))}
  ...
  color={incident.status === 'resolved' ? 'success' : incident.status === 'investigating' ? 'warning' : 'error'}
  />,
  </TableCell>,
  <TableCell>{formatTimestamp(incident.createdAt)}</TableCell>,
  <TableCell>,
  ...
  label={vuln.severity}
  size="small",
  className={getSeverityClass(vuln.severity)}
  />,
  </Box>,
  <Box textAlign="right">,
  ...
  onClick={() => window.open(vuln.patchUrl, '_blank')}
  >,
  View Patch,
  </Button>,
  <Button,
  ...
  className={getSeverityClass(selectedIncident.severity)}
  />,
  </Grid>,
  <Grid item xs={12}>,
