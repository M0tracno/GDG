...
foreach ($file in $fileList) {
    ...
    if (Test-Path $filePath) {
        ...
        # Check if the file has makeStyles but not the import
        if ($content -match "makeStyles" -and -not ($content -match "import makeStyles")) {
            ...
            $updatedContent = $content -replace "(import .+?;)", "`$1`nimport makeStyles from '$importPath';"
